package ar.edu.utn.dds.bot.modules.fuente;
...
public class FuenteCommand implements Command {
    @Override public String name() { return "FUENTE"; }
    @Override
    public void execute(Update update, DdsBot bot) throws Exception {
        Long chatId = update.getCallbackQuery().getMessage().getChatId();
        bot.notifyCallbackOk(update);
        bot.execute(SendMessage.builder().chatId(chatId.toString()).text(Texts.FUENTE_WELCOME + "\n(Pronto: submenú, acciones, etc.)").build());
    }
}
