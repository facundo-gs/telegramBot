package ar.edu.utn.dds.bot.modules.agregador;
...
public class AgregadorCommand implements Command {
    @Override public String name() { return "AGREGADOR"; }
    @Override
    public void execute(Update update, DdsBot bot) throws Exception {
        Long chatId = update.getCallbackQuery().getMessage().getChatId();
        bot.notifyCallbackOk(update);
        bot.execute(SendMessage.builder().chatId(chatId.toString()).text(Texts.AGREGADOR_WELCOME + "\n(Pronto: submenú, acciones, etc.)").build());
    }
}
